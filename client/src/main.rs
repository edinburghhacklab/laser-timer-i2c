...
fn read<T: I2c>(i2c: &mut T, addr: u8, cmd: u8, len: usize, units: &str) {
	...
	if result.is_ok() {
		...
		println!("0x{addr:02x} 0x{cmd:02x} {result:?} {buf:?} {value}{units}");
	} else {
...
fn main() {
	...
	read(&mut i2c, I2C_SLAVE_ADDRESS, I2C_CMD_READ_LASER_TIME_US, 8, "µs");
	read(&mut i2c, I2C_SLAVE_ADDRESS, I2C_CMD_READ_LASER_TIME_MS, 7, "ms");
	read(&mut i2c, I2C_SLAVE_ADDRESS, I2C_CMD_READ_LASER_TIME_DS, 6, "ds");
}
